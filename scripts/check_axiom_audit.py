#!/usr/bin/env python3
"""Reject missing audit entries and nonstandard theorem dependencies."""
...
AUDIT_SOURCE = ROOT / "lean" / "AxiomAudit.lean"
AUDIT_OUTPUT = ROOT / "build" / "axiom-audit.txt"
ALLOWED_AXIOMS = {"propext", "Classical.choice", "Quot.sound"}


def fail(message: str) -> None:
    print(f"axiom audit failed: {message}", file=sys.stderr)
    raise SystemExit(1)
...
if not expected:
    fail("AxiomAudit.lean contains no '#print axioms' declarations")
...
print(
    f"axiom audit passed: {len(expected)} theorems; allowed dependencies: "
