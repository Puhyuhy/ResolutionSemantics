...
The quantitative pair bound remains exposed by
\leanname{ResolutionSemantics.finiteSeparationBound}; the intrinsic
observer-class statement is
\leanname{ResolutionSemantics.MasterTheorems.relativeFiniteComplementSeparation}.
...
Lean checks the explicit carrier-level counterexample as
\leanname{ResolutionSemantics.RelativeObservers.finiteRelativeComplement_not_closed_under_binary_products}.
Thus the usual residual argument that combines finitely many pairwise
separators by taking their Cartesian product does not stay inside the present
observer class when the preserved base is infinite.  The direct finite-pattern
construction in \cref{thm:finite-pattern-realization} is therefore not replaced
by that standard product trick.
