...
\]
Thus $(\theta_m)$ defines a Hausdorff congruence uniformity on $F(D)$, in the
standard sense of congruence topologies on universal algebras
\cite{BulmanFleming1971}.  Because the basis is countable and nested, the
uniformity is metrizable; consequently Cauchy sequences suffice to describe its
completion.
...
collapsed ideal class while leaving $x$ singleton.  The Hausdorff completion
...
is different in that the retained carrier is only a partial base.  More
precisely, the embedded copy of $A$ is closed under the total operations of
$F(D)$ if and only if $D$ was already total: any undefined basic application
produces a non-base normal term immediately outside $A$.
